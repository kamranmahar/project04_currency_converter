...
let currencyRate:currencyConversion[]=[{name:"Euro",rate:0.936156},
            {name:"USD",rate:1},
...
let getRate=(curency:string):number=>{
   let rate= currencyRate.find((obj)=> {return obj.name==curency[0]});
   if(rate!=undefined)
        return rate.rate
   return 0;
}
let calculateConversion=(from:string,to:string,amount:number):number=>{
...
let curency:curencies= await inquirer.prompt([{
    name:"fromcurrency",
    type:'checkbox',
